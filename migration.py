...
#file_test
    ...
    print 'broker_id_survivor: %s' % broker_id_sur
    print 'partition_id on die broker: %s' % partition_id
    print 'broker not on line: %s' % broker_id_die
    print 'partition_on_broker_map: %s' % partition_on_broker
    ...
    if len(partition_id) != 0:
        print "leader's value is '-1' partition_id: %s" % partition_id
...
def mapping_(partition_on_broker, partition_id):
    ...
    print 'sorted_tuple: %s' % sorted_tuple
    for i in range(lens):
